...
@triton.jit
def _bicubic2d_affine_packed_accumulate_kernel(
    coeff_ptr,
    affine_ptr,
    dose_sum_ptr,
    stride_b,
    ...
    """Sample, positive-clip, scale and accumulate one packed ROI."""
    offs = tl.program_id(0) * BLOCK_SIZE + tl.arange(0, BLOCK_SIZE)
    mask = offs < N
    iz = offs % roi_z
    q = offs // roi_z
    ...
    z = iz.to(tl.float32)
    tq = (
        tl.load(affine_ptr + 0) * x
        + tl.load(affine_ptr + 1) * y
        + tl.load(affine_ptr + 2) * z
        + tl.load(affine_ptr + 3)
    )
    hq = (
        tl.load(affine_ptr + 4) * x
        + tl.load(affine_ptr + 5) * y
        + tl.load(affine_ptr + 6) * z
        + tl.load(affine_ptr + 7)
    )
    wq = (
        tl.load(affine_ptr + 8) * x
        + tl.load(affine_ptr + 9) * y
        + tl.load(affine_ptr + 10) * z
        + tl.load(affine_ptr + 11)
    )
    nT = UPSCALE_FACTOR * nTc
    nH = UPSCALE_FACTOR * nHc
    wi = tl.floor(wq + 0.5).to(tl.int32)
    ...
    for a in range(4):
        ...
        for htap in range(4):
            ...
            acc += wt * wh * tl.load(
                coeff_ptr + src, mask=mask & valid, other=0.0
            )

    destination = (
        ((ix + start_x) * plan_y + (iy + start_y)) * plan_z
        + iz
        + start_z
    )
    previous = tl.load(
        dose_sum_ptr + destination, mask=mask & valid, other=0.0
    )
    contribution = tl.maximum(acc, 0.0) * scale
    tl.store(
        dose_sum_ptr + destination,
...
def bicubic_iir_accumulate_affine_from_packed_coeff_triton(
    packed: torch.Tensor,
    affine: torch.Tensor,
    dose_sum: torch.Tensor,
    roi_box: tuple[int, int, int, int, int, int],
    *,
    scale: float = 1.0,
) -> None:
    """Directly accumulate a positive-clipped packed spline into a CT plan.

    This inference-only path supports one control point per launch. It avoids
    materializing both the sampled ROI and its validity mask.
    """
    if packed.ndim != 5 or not packed.is_cuda:
        raise ValueError(
            "packed must be a CUDA tensor with shape (1,T,r**2,H,W)"
        )
    if packed.shape[0] != 1:
        raise ValueError("direct plan accumulation currently requires batch size 1")
    ...
    if (
        ...
        raise ValueError(
            "packed coefficient channels must be a square of an upscale "
            f"factor >= 2, got {packed.shape[2]}"
        )
    if dose_sum.ndim != 3 or not dose_sum.is_cuda:
        raise ValueError("dose_sum must be a CUDA (X,Y,Z) tensor")
    if dose_sum.dtype != torch.float32 or not dose_sum.is_contiguous():
        raise ValueError("dose_sum must be contiguous float32")
    if dose_sum.device != packed.device:
        raise ValueError("packed and dose_sum must be on the same CUDA device")
    affine_c = affine.to(
        device=packed.device, dtype=torch.float32
    ).contiguous()
    if affine_c.shape != (1, 3, 4):
        raise ValueError(f"expected affine (1,3,4), got {tuple(affine_c.shape)}")

    xs, xe, ys, ye, zs, ze = (int(value) for value in roi_box)
    plan_x, plan_y, plan_z = (int(value) for value in dose_sum.shape)
    if not (
        0 <= xs < xe <= plan_x
        and 0 <= ys < ye <= plan_y
        and 0 <= zs < ze <= plan_z
        ...
        raise ValueError(
            f"roi_box {roi_box} is outside plan shape {tuple(dose_sum.shape)}"
        )
    roi_x, roi_y, roi_z = xe - xs, ye - ys, ze - zs
    total = roi_x * roi_y * roi_z
    _, nTc, _, nHc, nW = packed.shape
    _bicubic2d_affine_packed_accumulate_kernel[
...
def bicubic_zcollapsed_sample(
    ...
    *,
    ...
    """Use Triton by default, or select CuPy as an explicit fallback."""
    if backend == "triton":
        return bicubic_zcollapsed_sample_triton(coeff, points_th, target_w_index, zq)
    if backend == "cupy":
        from patient_space_resample_cuda import bicubic_zcollapsed_sample_cuda

        return bicubic_zcollapsed_sample_cuda(coeff, points_th, target_w_index, zq)
